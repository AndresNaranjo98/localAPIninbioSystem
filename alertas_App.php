...
$tipoAlerta = filter_var($jsonRegistros->tipoAlerta, FILTER_SANITIZE_NUMBER_INT);
$tequilera = filter_var($jsonRegistros->tequilera, FILTER_SANITIZE_NUMBER_INT);
$ultimoDato;
$penultimoDato;
$concat = "";
$temperatura;
...
$tinas->bindParam(':tequilera', $tequilera);
$tinas->execute();
...
while ($rows = $limites->fetchAll(PDO::FETCH_ASSOC)) {
    ...
    if ($tipoAlerta == 1) {
        ...
        $statement1->bindParam(':tina', $tina);
        $statement1->bindParam(':tequilera', $tequilera);
        $statement1->execute();
